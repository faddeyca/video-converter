...
def process_video(speed=1,
                  ...
                  hw=None):
    '''
    Извлекает кадры из текущего видео,
    делает с ними какие-то действия и записывает новое видео.

        Параметры:
            speed: Скорость нового видео.
            По умолчанию 1 - скорость не изменяется.

            funcIndex: Функция.
                Выполняется на каждой итерации.
                Принимает на вход индекс текущего кадра.
                Возвращает булевое значение,
                означающее нужно ли вписывать текущий кадр в новое видео.
            ...
            funcBegin: Функция.
                Выполняется один раз, до начала извлечения кадров.
            ...
            hw: Расширение нового видео.
                Если пустая,
                то новое видео имеет такое же расширение как и текущее.

            Возвращает:
                Количество кадров в новом видео.
                Иногда используется,
                так как при сложных манипуляциях трудно посчитать
                количество кадров в новом видео.
    '''
    ...
    #  Создание нового видео с параметрами
    new_video = cv2.VideoWriter(
        str(Path("temp/temp.mp4")), cv2.VideoWriter_fourcc(*"mp4v"),
        frames_per_sec * speed, (width, height))

    #  Выполнение funcBegin, если не пустая
    if funcBegin is not None:
        funcBegin(frame)

    #  Индекс текущего кадра
    index = 0
    #  Счётчик количества кадров в новом видео
    count = 0

    #  Выполняется пока извлечение кадров успешно
    while ok:
